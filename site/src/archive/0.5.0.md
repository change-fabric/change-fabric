...
```
---
spec_version: "0.5.0"   # optional: the schema version this file was authored against
change_config:
...
## contributors_team fields (0.5.0)
...
under `artifacts`, with that team's shared findings-artifact area.

`artifacts` is the switch for the whole artifact pipeline. With it present, a
...
viewport, then publishes the bundle to the team's private S3 bucket behind a
CloudFront distribution and rebuilds the team index page listing every
contributor's runs. Without it, none of that happens and the sweep behaves
exactly as it did in 0.4.0: no media is captured, no bundle is built, nothing
is uploaded.

Provisioning is one human-run command, `ruby scripts/cf_artifacts_init.rb
<team_id>`, which creates the bucket, the distribution, the basic-auth
CloudFront function, and the manifest table, then prints the `artifacts:` block
below filled in. See `scripts/CF_TEAM_SETUP.md` for the runbook.

**Nothing under `artifacts` is a credential.** `basic_auth` names where the
viewer credential lives (an SSM SecureString parameter, a 1Password item
reference), never what it is, the same indirection
`lanes.<lane>.basic_auth.username_env` already uses.

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `contributors_team.team_id` | string | yes, to register a team | Stable id of the contributors team this repo belongs to, minted by `cf_team_init.rb`. |
...
| `contributors_team.contributors[].name` | string | yes, on each entry | Display name, shown as the run's author on published artifacts and in the team index. |
| `contributors_team.artifacts.enabled` | boolean | no (default true) | Set false to keep the block committed but publish nothing, for example while a bucket is being migrated. |
| `contributors_team.artifacts.bucket` | string | yes, to publish | The team's private artifact bucket. Its presence is what enables the pipeline; an empty or absent value is treated as "not configured". |
| `contributors_team.artifacts.region` | string | no (default `us-east-1`) | Region of the bucket and the manifest table. |
| `contributors_team.artifacts.aws_profile` | string | no (default `personal`) | Local AWS profile used to publish, matching `cf_team_init.rb`'s convention. `AWS_PROFILE` in the environment overrides it. |
| `contributors_team.artifacts.distribution_id` | string | no | CloudFront distribution id, used only to invalidate the two rewritten index objects after a publish. Omitted, the upload still succeeds and the index simply serves from cache until its TTL expires. |
| `contributors_team.artifacts.domain` | string | no | The distribution's domain. Without it a run publishes but no viewer url can be printed, since a private bucket has no reachable url of its own. |
| `contributors_team.artifacts.manifest_table` | string | no (default `cf-change-artifacts`) | DynamoDB table holding one row per published run, the durable source the team index page is rebuilt from. |
| `contributors_team.artifacts.basic_auth.username` | string | no (default `cf`) | Username half of the viewer credential the CloudFront function checks. Documentation for humans; the function compares a digest of the whole header. |
| `contributors_team.artifacts.basic_auth.ssm_parameter` | string | no | Name of the SSM SecureString parameter holding `username:password`. This is the credential's source of truth: `cf_artifacts_init.rb` reads it at deploy time and compiles only its SHA-256 digest into the function, because a CloudFront function has no network access and cannot fetch a secret at request time. |
| `contributors_team.artifacts.basic_auth.secret_ref` | string | no | A 1Password `op://` reference for the same credential, so a teammate can retrieve it without AWS access. |
| `contributors_team.artifacts.media.screenshots` | boolean | no (default true) | Whether the browserless lane captures a full-page screenshot per route and viewport for the artifact. |
| `contributors_team.artifacts.media.video` | boolean | no (default true) | Whether the browserless lane records one video per viewport covering that viewport's whole route walk. |
| `contributors_team.artifacts.media.video_fps` | integer | no (default 6) | Frame rate of that recording. Higher is smoother and proportionally larger; the recording travels base64 inside the lane's single browserless response, so this is the main lever on that payload. |
...
contributors_team:
  ...
  artifacts:
    bucket: cf-change-artifacts-acme-web
    region: us-east-1
    distribution_id: E1XXXXXXXXXXXX
    domain: d111111abcdef8.cloudfront.net
    basic_auth:
      username: cf
      ssm_parameter: /cf-change-artifacts/acme-web/basic-auth
      secret_ref: "op://Shared/change-fabric artifacts: acme-web/password"
```
