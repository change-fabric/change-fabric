...
class ClientNameGuardTest < Minitest::Test
  ...
  def guard(terms_path, tool_name, tool_input, remote_v: CF_REMOTE)
    io = StringIO.new
    ClientNameGuard.new({ "tool_name" => tool_name, "tool_input" => tool_input },
                        terms_path: terms_path, remote_v: remote_v).emit(io)
    return nil if io.string.empty?

    JSON.parse(io.string)["hookSpecificOutput"]
  end

  def decision(terms_path, tool_name, tool_input, remote_v: CF_REMOTE)
    guard(terms_path, tool_name, tool_input, remote_v: remote_v)&.dig("permissionDecision")
  end
  ...
  def test_allows_flagged_term_outside_the_cf_toolkit_repo
    with_terms_file("acme corp\n") do |path|
      assert_nil decision(path, "Write", { "content" => "Fixed the Acme Corp login bug." }, remote_v: CLIENT_REMOTE)
    end
  end

  def test_denies_flagged_term_inside_the_cf_toolkit_repo
    with_terms_file("acme corp\n") do |path|
      assert_equal "deny", decision(path, "Write", { "content" => "Fixed the Acme Corp login bug." }, remote_v: CF_REMOTE)
    end
  end

  def test_never_shells_out_when_there_is_no_match
    called = false
    probe = lambda do
      called = true
      CF_REMOTE.call
    ...
    with_terms_file("acme corp\n") do |path|
      assert_nil decision(path, "Write", { "content" => "plain generic text" }, remote_v: probe)
    end
    refute called, "the repo remote should only be checked once a term actually matches"
  end
end
