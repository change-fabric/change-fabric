...
`CHANGE.md` reads this to get every field right without reverse-engineering
the parser. The field set and version here are kept honest by
`test/change_schema_spec_test.rb`, which fails if this document and the
parsing code (`scripts/change_schema.rb`) drift.
...
```
---
spec_version: "0.9.0"   # optional: the schema version this file was authored against
change_config:
...
- `<lane>`: any of the five lanes, `k6`, `a11y`, `zap`, `browserless`, `testcases`.
- `<ref>`: a promotion target (0.9.0; formerly `<branch>`, since only
...
| `change_config.lanes.<lane>.wait_for.selector` | string | no (default `body`) | (0.10.0) The readiness contract a browser lane (`a11y`, `browserless`, `testcases`) waits on after a navigation, before it grades the page: a selector that must exist. Written as a bare string (`wait_for: "[data-test=ready]"`) this is the field it fills. It replaces the old bare `networkidle2`, a timing heuristic that passes early on a page still hydrating and hangs on one holding an open socket. The wait condition is the contract; the timeout below is only the failure mode. |
| `change_config.lanes.<lane>.wait_for.load_state` | enum `load` `domcontentloaded` `networkidle0` `networkidle2` | no (default `domcontentloaded`) | (0.10.0) The Puppeteer load state the navigation itself waits for. An unrecognized value falls back to the default rather than being handed to Puppeteer, which would read as a navigation error on every route. |
| `change_config.lanes.<lane>.wait_for.url_contains` | string | no | (0.10.0) A fragment the browser's final url must contain before the page is graded, for a route that is only ready once a client-side redirect has settled. |
| `change_config.lanes.<lane>.wait_for.url_matches` | string | no | (0.10.0) A regular expression the browser's final url must match, for the same reason as `url_contains` when a fragment is not precise enough. |
| `change_config.lanes.<lane>.wait_for.timeout_ms` | integer | no (default 30000) | (0.10.0) How long the readiness contract may take before it is declared unmet. A bound on failure, not the definition of ready. |
| `change_config.lanes.<lane>.retries` | integer | no (default 0) | (0.10.0) How many times to re-run this lane while it is failing. Zero by default, and deliberately: a gate that quietly re-runs until green is not a gate. A finding that only stopped failing on a later attempt keeps its real status and is recorded `flaky` beside it, with the attempt count, so a pass bought by a re-run is legible as one. |
...
| `change_config.lanes.browserless.routes[].wait_for` | string or mapping | no (default the lane's own `wait_for`) | (0.10.0) This route's own readiness contract, same shape as `lanes.<lane>.wait_for` above (a bare string is the selector), overriding the lane-wide one field by field. A route that means something specific by "ready" states it here rather than lengthening every other route's wait. |
| `change_config.lanes.browserless.cell_isolation` | boolean | no (default true) | (0.10.0) Whether each viewport-by-route cell gets its own browser context. Isolation is the default because a cell's verdict should depend on the cell, not on whichever cell ran before it and left a cookie, a scroll position, or a dismissed-banner flag behind. Set false only for a matrix whose routes are steps of one stateful flow, where carrying the session forward is the point; that restores the old shared-page behavior, including a single login for the whole run. |
| `change_config.lanes.browserless.locale` | string | no (default `en-US`) | (0.10.0) The locale pinned for every cell, as both the `Accept-Language` request header and what the page's own JS reads back from `navigator.language`. Pinned rather than inherited so a rendering does not depend on whichever machine ran the sweep. |
...
### change_config.lanes.testcases (0.10.0): committed regression cases

The fifth lane runs test cases that live in the repo. A case is a suite id, a
tag list, a human-prose `acceptance` sentence, and an ordered list of
declarative steps (`goto`, `click`, `fill`, `expect_visible`, `expect_url`, and
the rest). Ruby compiles those steps into the browserless payload, so the same
case at the same commit gives the same verdict; nothing is generated at run
time. The lane shares the browserless container the other browser lanes already
use and adds no image.

Cases are never written in `CHANGE.md`. They live in sidecar suite files beside
the code they test (conventionally `<dir>/<name>.cf-testcases.yml`), referenced
here by glob, and carry their own schema, documented in
`skills/testcases/reference/suite-file-spec.md`. What belongs in this file is
only which suites this repo gates on and where they are pointed. A failing case
fails the lane, and therefore the merge and tag gates, exactly like every other
lane; `gate_tags` is the staged-adoption valve for a suite still being trusted.

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `change_config.lanes.testcases.suites` | list of string | yes, to run the lane | Repo-relative globs naming the suite files to load (e.g. `qa/*.cf-testcases.yml`). A glob matching no readable file is an error, not an empty run: a suite list that silently resolves to zero cases is how a gate stops checking anything without anyone noticing. |
| `change_config.lanes.testcases.tags` | list of string | no (default every case) | Runs only cases carrying at least one of these tags, for a repo that wants its smoke tag on every merge and its full suite elsewhere. |
| `change_config.lanes.testcases.gate_tags` | list of string | no (default every case gates) | The tags whose cases may fail the gate. Omitted, every case gates, which is the point of writing one down. Naming tags here reports the rest without failing the run, the escape valve for adopting a brand-new suite in stages. An entry no loaded case carries is an error, since it silently changes what gates. |
| `change_config.lanes.testcases.viewport.name` | string | no (default `desktop`) | Label for the viewport every case runs at. |
| `change_config.lanes.testcases.viewport.width` | integer | no (default 1440) | Viewport width in pixels. Pinned at device scale factor 1, so a screenshot does not depend on the host. |
| `change_config.lanes.testcases.viewport.height` | integer | no (default 900) | Viewport height in pixels. |
| `change_config.lanes.testcases.timeout_ms` | integer | no (default 15000) | Default per-step timeout. A step may override it with its own `timeout_ms`. |
| `change_config.lanes.testcases.auth.login_url` | string | yes, to run a case behind a login (unless `auth.steps` is used instead) | The same login-flow block `lanes.browserless.auth` defines, field for field, because a case behind a login has to get through the same login. Login page path (relative to the lane's base url) or absolute url. |
| `change_config.lanes.testcases.auth.email_env` | string | yes, to log in (shorthand form) | Name of the environment variable holding the real test login email/username. The value is never written into `CHANGE.md`. |
| `change_config.lanes.testcases.auth.password_env` | string | yes, to log in (shorthand form) | Name of the environment variable holding the real test login password. Never written into `CHANGE.md`. |
| `change_config.lanes.testcases.auth.email_selector` | string | no (default `input[name="email"]`) | CSS selector for the email/username field (shorthand form). |
| `change_config.lanes.testcases.auth.password_selector` | string | no (default `input[type="password"]`) | CSS selector for the password field (shorthand form). |
| `change_config.lanes.testcases.auth.submit_selector` | string | no (default `button[type="submit"]`) | CSS selector for the login form's submit control (shorthand form). |
| `change_config.lanes.testcases.auth.wait_for_selector` | string | no | Selector to wait for after submit, confirming the post-login page rendered before any case runs (shorthand form). |
| `change_config.lanes.testcases.auth.timeout_ms` | integer | no (default 15000) | Timeout for each login step (shorthand form). |
| `change_config.lanes.testcases.auth.steps[].url` | string | yes, on the first step | Page to navigate to before filling this step's fields, for a login needing more than one form (an OTP flow). |
| `change_config.lanes.testcases.auth.steps[].fields[].selector` | string | yes | CSS selector for this step's input field. |
| `change_config.lanes.testcases.auth.steps[].fields[].env` | string | yes, unless `code_source` is set | Name of the environment variable holding this field's value. Never written into `CHANGE.md`. Mutually exclusive with `code_source`. |
| `change_config.lanes.testcases.auth.steps[].fields[].code_source.url` | string | yes, to use `code_source` | An HTTP endpoint reachable from the browserless container on the run network, polled for this field's value inside the container at fill time, never read or logged on the host. |
| `change_config.lanes.testcases.auth.steps[].fields[].code_source.pattern` | string | no | A regex applied to the endpoint's response body; the first capture group (or the whole match) becomes the field value. |
| `change_config.lanes.testcases.auth.steps[].fields[].code_source.timeout_ms` | integer | no (default 20000) | How long to keep polling before failing this login attempt. |
| `change_config.lanes.testcases.auth.steps[].fields[].code_source.poll_interval_ms` | integer | no (default 1000) | Delay between polling attempts. |
| `change_config.lanes.testcases.auth.steps[].submit_selector` | string | no (default `button[type="submit"]`) | CSS selector for this step's submit control. |
| `change_config.lanes.testcases.auth.steps[].wait_for_selector` | string | no | Selector to wait for after this step's submit. |
| `change_config.lanes.testcases.auth.steps[].timeout_ms` | integer | no (default 15000) | Timeout for this step's navigation, field waits, and post-submit wait. |

`lanes.testcases.base_url`, `lanes.testcases.basic_auth.*`, `lanes.testcases.retries`
and `lanes.testcases.wait_for.*` are the generic `lanes.<lane>.*` fields above,
and mean here exactly what they mean everywhere else.
...
| `change_config.profiles.<profile>.lanes.testcases.auth.email_selector` | string | no | (0.10.0) Overrides `testcases.auth.email_selector` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.password_selector` | string | no | (0.10.0) Overrides `testcases.auth.password_selector` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.submit_selector` | string | no | (0.10.0) Overrides `testcases.auth.submit_selector` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.wait_for_selector` | string | no | (0.10.0) Overrides `testcases.auth.wait_for_selector` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.timeout_ms` | integer | no | (0.10.0) Overrides `testcases.auth.timeout_ms` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].url` | string | no | (0.10.0) Overrides `testcases.auth.steps[].url` for this profile (multi-step login). |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].fields[].selector` | string | no | (0.10.0) Overrides `testcases.auth.steps[].fields[].selector` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].fields[].env` | string | no | (0.10.0) Overrides `testcases.auth.steps[].fields[].env` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].fields[].code_source.url` | string | no | (0.10.0) Overrides `testcases.auth.steps[].fields[].code_source.url` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].fields[].code_source.pattern` | string | no | (0.10.0) Overrides `testcases.auth.steps[].fields[].code_source.pattern` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].fields[].code_source.timeout_ms` | integer | no | (0.10.0) Overrides `testcases.auth.steps[].fields[].code_source.timeout_ms` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].fields[].code_source.poll_interval_ms` | integer | no | (0.10.0) Overrides `testcases.auth.steps[].fields[].code_source.poll_interval_ms` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].submit_selector` | string | no | (0.10.0) Overrides `testcases.auth.steps[].submit_selector` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].wait_for_selector` | string | no | (0.10.0) Overrides `testcases.auth.steps[].wait_for_selector` for this profile. |
| `change_config.profiles.<profile>.lanes.testcases.auth.steps[].timeout_ms` | integer | no | (0.10.0) Overrides `testcases.auth.steps[].timeout_ms` for this profile. |
...
### change_policy tag rules (0.9.0): trunk with environment tags
...
is empty when a repo declares none, and `change_tag_guard.rb` is inert
whenever that set is empty.

**Enforcement and run ergonomics.** `change_tag_guard.rb`, a `PreToolUse`
hook, gates the tag *push* (`git push` of a matching tag, or `gh release
create`), sharing its gate question with `change_merge_guard.rb` via
`ChangeGateCheck`. `change_run.rb all --for-tag <tagname>` resolves the tag
against `promotion`'s tag rules, sweeps once per profile they name, and
refuses to run against the wrong commit. `change_run.rb gate-status [--ref
REF]` is read-only: no docker, no boot, no lanes, just the matching rules and
whether each is already satisfied, for a tag name, a branch, or a SHA
(default `HEAD`) -- the same question the tag guard's own deny decision
answers, reproducible outside the hook.
...
| `change_policy.protected_refs` | list of string | no (default `[]`) | (0.9.0) Refs whose promotion is gated, each an unprefixed branch name, `branch:<name>`, or `tag:<glob>`. Unioned with `protected_branches` and with every key under `promotion:`. A repo with no `tag:` entry anywhere is unaffected: `protected_branches` alone still governs branch gating exactly as before this field existed. |
...
| `change_policy.promotion.<ref>.environment` | string | no (default the key itself) | (0.9.0) Human label for this promotion target, used in deny messages and `doctor` output. Useful when the pattern (`tag:release/*/v*`) does not read as an environment name. |
| `change_policy.promotion.<ref>.require_trunk_ancestor` | string | no | (0.9.0) Tag rules only. Branch the tagged commit must be an ancestor of (or equal to). Ignored, and flagged by `doctor`, on a branch rule. |
| `change_policy.promotion.<ref>.require_prior_tag` | string (glob) | no | (0.9.0) Tag rules only. A published tag matching this glob must already point at the same commit. |
| `change_policy.promotion.<ref>.require_testcases` | boolean | no (default false) | (0.10.0) Additionally requires the recorded comprehensive pass for this ref to carry a passing `testcases` lane. Opt-in and stricter than `require_change_pass` alone, which asks only whether the run passed overall and is satisfied by a run that never ran the lane. Fails closed: a record with no entry for the lane is not a pass. |
...
- 0.9.0: trunk + tag releases. `change_policy.promotion` accepts a
  `tag:`-prefixed key (`tag:staging/v*`) alongside branch keys, in the same
  map, so a repo with one trunk and no long-lived environment branches can
  gate a release tag exactly as a branch-per-environment repo gates a merge.
  `change_policy.protected_refs` names branches and `tag:<glob>` patterns
  together, unioned with `protected_branches` (retained, unchanged). Every
  field a branch rule carries means the same thing on a tag rule; two new
  fields, `require_trunk_ancestor` and `require_prior_tag`, restore what
  branch topology encodes structurally (a commit's promotion order and
  provenance) for a topology that has to state it instead.
  `change_policy.promotion.<ref>.environment` is a human label for a
  promotion target, used in deny messages and `doctor` output.
  `change_tag_guard.rb`, a new `PreToolUse` hook, gates the tag *push* (a
  matching `git push`, or `gh release create`), sharing its gate question
  with `change_merge_guard.rb` through the new `ChangeGateCheck`.
  `change_run.rb all --for-tag <tagname>` resolves a tag against its
  matching rules and sweeps the right profile(s) against the right commit;
  `change_run.rb gate-status [--ref REF]` answers, read-only, whether a ref's
  matching rules are already satisfied. A repo with no `tag:` key anywhere is
  bit-for-bit unaffected: no new field is required, no existing accessor's
  contract changes, and `change_tag_guard.rb` never fires.
- 0.10.0: `testcases`, a fifth lane, and the fields that point it at committed
  regression cases. `change_config.lanes.testcases.suites` is a list of globs
  naming sidecar suite files that live beside the code they test; cases are
  never inline in `CHANGE.md`, and the suite file carries its own schema, its
  own doc (`skills/testcases/reference/suite-file-spec.md`) and its own drift
  test, because it is a different document with a different lifecycle and a
  new step verb should not need a `CHANGE.md` spec bump. `tags` selects which
  cases run and `gate_tags` selects which may fail the gate; omitted, every
  case gates, since a test case that cannot fail the build is a comment.
  `viewport.*`, `timeout_ms` and the full `auth.*` login block (the same shape
  `browserless.auth` already defines, because a case behind a login has to get
  through the same login) complete the lane, and a profile may override
  exactly `base_url`, `basic_auth.*` and `auth.*`, the same where-not-what rule
  0.8.0 already applied to `browserless.auth`. `targets` is deliberately not
  accepted: the lane reads no target list, and a scope nothing reads is worse
  than no scope. `change_policy.promotion.<ref>.require_testcases` lets a
  branch additionally require the recorded comprehensive pass to carry a
  passing `testcases` lane, which `require_change_pass` alone does not ask.
  This version also documents four fields that were already live and
  undocumented, which is the drift the registry exists to prevent:
  `lanes.<lane>.wait_for.*` (the explicit per-route readiness contract that
  replaced a bare `networkidle2` timing heuristic, including
  `browserless.routes[].wait_for`), `lanes.<lane>.retries` (an opt-in retry
  budget, 0 by default, recording `flaky` beside a real status rather than
  laundering a retried pass into a clean one),
  `lanes.browserless.cell_isolation` (a fresh browser context per
  viewport-by-route cell, so a cell cannot inherit its neighbour's cookies or
  scroll position), and `lanes.browserless.locale` (a pinned locale, so a
