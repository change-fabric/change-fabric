...
- **What `install.rb` actually does** (read in full). Its `Installer#install` runs, in order: `place_hooks` (wipes `~/.claude/cf/bin`, then copies every `scripts/*.rb` in and `chmod 0755`), `pin_expected_home` (writes `~/.claude/cf/bin/.expected-home` with `Dir.home`), `link_skills` (symlinks `~/.claude/skills/<cf:name>` at each `skills/*` source dir inside the repo checkout, and prunes stale links it owns, including a legacy `~/.claude/pst/bin` from before the pst-to-cf rename), `link_pi_extension` (symlinks the Pi hook extension), `mirror_to_pi` (prunes legacy Pi skill copies, appends `~/.claude/skills` to `~/.pi/agent/settings.json`'s `skills` array), `mirror_to_opencode` (copies a JSONC-safe, renamed `cf-foo` mirror of each skill into `~/.config/opencode/skills` and registers the path in `~/.config/opencode/opencode.jsonc`), and `wire_settings` (rewrites `~/.claude/settings.json` `hooks` for six events, keeping a `.bak`).
- **The critical asymmetry.** Hooks are copied; skills are symlinked into the repo checkout. `settings.json` hook commands are absolute paths into `~/.claude/cf/bin` (stable, home-relative). But `~/.claude/skills/*` symlinks point at `<repo>/skills/*`. The repo checkout must persist at a stable path for the install to keep working. This single fact dominates the Homebrew fit question.
...
- **Runtime dependencies.** Ruby (CI pins 3.4) plus gems `rake`, `csv`, `rubocop-rails-omakase` (`Gemfile`); Node is only a dev toolchain (`package.json` devDependencies: `@earendil-works/pi-coding-agent`, `typescript`, `@types/node`) used for typecheck and the Pi extension build, not required at skill-runtime. Docker is a hard runtime dependency for the `cf:change` lanes (k6/a11y/zap/browserless all run dockerized), and `docker_doctrine_guard.rb` actively polices host daemons.
- **Versioning, two independent axes.** `VERSION` (currently `0.30.0`) versions the whole toolkit; tags are `skills/vX.Y.Z` (renamed this session from bare `vX.Y.Z`). `ChangeSchema::VERSION` (currently `0.4.0`, in `scripts/change_schema.rb`) versions the CHANGE.md frontmatter schema independently; tags are `change-schema/vX.Y.Z`. The schema spec (`skills/change/reference/CHANGE-frontmatter-spec.md`, "Versioning and changelog") defines a `-alpha.N`/`-beta.N` SemVer 2.0.0 prerelease convention, currently used only for the schema, not for `skills/vX.Y.Z`. The 0.4.0 schema bump added a monorepo `change_config.apps` registry; it changes what `cf:change` audits per app, not anything this Homebrew plan's install/uninstall or tap mechanics touch.
...
- No unusual external runtime requirements. Core is hostile to formulae that only function alongside a heavy out-of-band runtime. `cf:change` requires Docker at runtime; expressing that in core would demand awkward conditionals or a caveats-only hand-wave that core reviewers dislike.
...
```ruby
url "https://github.com/pstaylor-patrick/change-fabric/archive/refs/tags/skills/v0.30.0.tar.gz"
sha256 "..."
```

Note the tag contains a slash (`skills/v0.30.0`); GitHub's archive URL accepts it as a path segment and the resulting tarball's top-level dir is `change-fabric-skills-v0.30.0`. The formula's `install` uses `Dir["*"]` off the extracted root, so the odd dirname is irrelevant. This is worth calling out because the slash-in-tag is unusual and a hand-written url is easy to get wrong; the automation in section 4 should construct it mechanically.
...
- Merge mode (the `cf` skill: local-only, merge-ready, admin-bypass, yolo) is per-session and set inside a Claude Code session, never at install time. The formula must not presume a mode.
...
- `cf:change` runs, `cf:drive`, review gates, and ctx captures are all invoked on demand during work, never at install.

The formula's `caveats` string should state, in plain text (glyph-clean, per `CLAUDE.md`): that hooks and skills are now wired into `~/.claude/`; that a backup of the prior `settings.json` is at `~/.claude/settings.json.bak`; that `cf:change` needs Docker running; and that merge mode is chosen per session, not here.
...
`brew uninstall` removes the Cellar `libexec`. That leaves behind, unmanaged by Homebrew: the copied hooks in `~/.claude/cf/bin`, the `~/.claude/settings.json` hook wiring that references them, the `~/.claude/skills/*` symlinks (now dangling into the removed Cellar dir), the Pi/OpenCode registrations, and `.expected-home`. `install.rb` has no reverse operation. Options:

- First cut, recommended: document, don't automate the reversal. Homebrew's `caveats` text can tell the user how to fully unwind: restore `~/.claude/settings.json` from the `.bak` the installer kept, and remove `~/.claude/cf/bin` and the `~/.claude/skills/cf:*` links. This matches the repo's demonstrated preference for not building speculative infra, and it is honest: a clean automated uninstall needs an `install.rb --uninstall` that doesn't exist yet.
